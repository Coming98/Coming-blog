...
android:gravity="bottom|center"/> <!--可以同时指定多个值-->
~~~
...
~~~java
imageViewQQ.setImageResource(R.drawable.icon_github); // 修改图片源
~~~
...
区分：invisible不可见但是占有原来的位置，gone不可见也不占有位置

代码控制：

~~~java
String[] btnName = {"gone", "visible", "invisible"};
int[] state = new int[]{View.INVISIBLE, View.GONE, View.VISIBLE};
buttonTriggerProgressBar.setText(btnName[pbState]);
progressBar.setVisibility(state[pbState]);
pbState = (pbState + 1) % 3;
~~~
...
1. 通过AlertDialog.Builder创建一个AlertDialog的实例

~~~java
AlertDialog.Builder dialog = new AlertDialog.Builder(MainActivity.this);
~~~

2. 设置标题，内容，能否用Back键关闭对话框等属性

~~~java
dialog.setTitle("Alert Dialog");
dialog.setMessage("Do you want to leave CJC?");
dialog.setCancelable(false);
~~~

3. 创建Positive与Negative基础点击事件
